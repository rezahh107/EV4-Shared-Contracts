...
| `PG-SCHEMA-001` | Project Gate must not copy specialist schemas as competing canonical contracts. | `AGENTS.md`, `docs/VALIDATION_STRATEGY.md`, `schemas/README.md`, workflow schema allowlist. | Workflow `find schemas` allowlist. | CI checks current `schemas/` allowlist. | Final post-Inspector CI must pass before merge. | Specialist schemas remain in owner repos. | `fixture_tested` until final post-Inspector CI passes. | Extend scanner to future contract folders and lock manifests. |
| `PG-EVIDENCE-001` | No accepted/final result without explicit evidence. | `docs/RESULT_MODEL.md`, `docs/STATUS_DECISION_MATRIX.md`, Stage Bundle schema, validators, docs. | `BundleValidator`; `transition-result.v1` now requires non-null hashes/provenance for `accepted` and blocks warning/error/insufficient_evidence diagnostics under `accepted`. | Result status-correlation negative tests added for accepted-with-error, accepted-without-hash/provenance, repair/insufficient/invalid contradictions. | Final post-Inspector CI must pass before merge. | Limited to Stage Bundle and A2C synthetic fixtures. | `fixture_tested` at carrier level; full future transition evidence acquisition remains a gap. | Add transition-specific accepted-gate validators when CE→Builder/Builder→Responsive/final gates are implemented. |
| `PG-SYNTH-001` | Synthetic fixtures must not be treated as real EV4 evidence. | README, AGENTS, Stage Bundle `synthetic`, docs. | Stage Bundle schema requires top-level `synthetic`. | Prompt 01 adds missing synthetic-label negative fixture test. | Final post-Inspector CI must pass before merge. | No real downstream evidence. | `fixture_tested` at carrier level. | Add report-level guard that blocks real-readiness wording when `synthetic=true` or verification is `synthetic_fixture_only`. |
...
advanced:
  ...
  - PG-STATUS-001: target status presentation mapping and result schema status/diagnostic correlation added.
  - PG-UNICODE-001: explicit no-hidden-Unicode-normalization test added.
  - PG-EVIDENCE-001: result model/status decision carriers added; accepted carrier now requires explicit hash/provenance evidence.
...
not_advanced_to_full_enforcement:
  ...
  - final post-Inspector CI result must be checked before merge
```

## Inspector follow-up

`PRF-001` from the first PR Inspector pass reported that `validate_lock_manifest()` was weaker than `schemas/lock-manifest/lock-manifest.v1.schema.json` and `docs/LOCK_MANIFEST_POLICY.md`. The validator was updated to reject empty `files`, malformed `repository`, malformed `accepted_commit`, malformed `sha256_file_bytes`, and invalid `size_bytes`; negative tests were added in `tests/unit/test_prompt01_lock_manifest_strictness.py`.

The second PR Inspector pass reported that `transition-result.v1` accepted contradictory `accepted` payloads and that `validate_lock_manifest()` still accepted unknown fields despite schema `additionalProperties: false`. The result schema now enforces status/diagnostic/evidence correlation, the accepted fixture carries explicit provenance, and lock manifest unknown-field tests were added.
