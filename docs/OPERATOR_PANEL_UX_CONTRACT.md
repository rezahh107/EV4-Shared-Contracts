...
UI باید از semantic tokens در `src/ev4_transition/presentation/theme_tokens.py` استفاده کند. status باید icon + label + semantic tone داشته باشد، نه color-only. focus ring، code background و control indicator tokenها اجباری هستند.
...
- Dark Mode header نباید white/light island ایجاد کند؛ header باید از dark raised/overlay surfaceهای همان theme استفاده کند.
...
- radio selected/unselected/focus states باید بدون اتکا به browser default قابل تشخیص باشند.
- disabled state نباید فقط با opacity نمایش داده شود.

## Control visibility rules

Radio controls باید در هر دو theme این وضعیت‌ها را واضح نشان دهند:

- unselected: مرز قابل دیدن روی background؛
- selected: checked background + dot مستقل؛
- hover: تغییر subtle اما detectable؛
- keyboard focus: ring مستقل با contrast کافی.

این polish از `EV4-Workbook-Jinja` فقط اصل‌های theme shell، dark surface layering، visible controls، focus-visible و readable Persian line-height را قرض می‌گیرد. کپی palette، gold/teal identity یا workbook-style educational layout مجاز نیست.
...
Coverage جدید باید radio border/background، radio checked dot/background و radio focus ring/surface را هم در حد token-level اندازه‌گیری کند.
