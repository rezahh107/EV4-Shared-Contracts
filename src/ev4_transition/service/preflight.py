...
_GITHUB_DESKTOP_FA = (
    "در GitHub Desktop: 1. از Current repository همین ریپو را انتخاب کن. "
    "2. Current branch را روی main بگذار. 3. Fetch origin را بزن. "
    "4. اگر Pull origin ظاهر شد، آن را هم بزن. 5. دوباره Preflight را اجرا کن."
)
...
def run_preflight(request: GateRequest) -> PreflightResult:
    ...
    checks.extend(_lock_manifest_checks(request.repo_paths, choice))
    required_files = _required_files_by_field(request.repo_paths, choice)
    for field in _FIELDS:
        checks.extend(_path_checks(request.repo_paths, field, field in required, required_files.get(field, ()), choice))
    checks.append(
        PreflightCheck(
...
def _looks_like_project_gate_result(value: dict[str, Any]) -> bool:
    ...
    return (
        isinstance(result_type, str) and result_type.startswith(("service_", "ui_"))
        or value.get("schema_version") in {"ev4-project-gate-ui-result.v1", "project-gate-service-result.v1"}
        or ("engine_result" in value and "transition_choice" in value and "diagnostics" in value)
    )
